...
import (
	...
	"github.com/ernado/tentacle/internal/ent"
...
// The TelegramBlobFunc type is an adapter to allow the use of ordinary function as a Querier.
type TelegramBlobFunc func(context.Context, *ent.TelegramBlobQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f TelegramBlobFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.TelegramBlobQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.TelegramBlobQuery", q)
}

// The TraverseTelegramBlob type is an adapter to allow the use of ordinary function as Traverser.
type TraverseTelegramBlob func(context.Context, *ent.TelegramBlobQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseTelegramBlob) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseTelegramBlob) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.TelegramBlobQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.TelegramBlobQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	switch q := q.(type) {
	case *ent.TelegramBlobQuery:
		return &query[*ent.TelegramBlobQuery, predicate.TelegramBlob, telegramblob.OrderOption]{typ: ent.TypeTelegramBlob, tq: q}, nil
